...
class UNET_AttentionBlock(nn.Module):
    ...
    # The parameter `n_embd` has been renamed to `d_embed` to clarify it represents the TOTAL embedding dimension.
    def __init__(self, n_head: int, d_embed: int, d_context=768):
        super().__init__()
        # The line `channels = n_head * n_embd` has been REMOVED.
        # `d_embed` IS the total number of channels.
        channels = d_embed
...
class UNET(nn.Module):
    def __init__(self):
        ...
        # THE CALLS TO UNET_AttentionBlock ARE NOW CORRECTED
        # We pass the full channel dimension (e.g., 320), not the dimension per head (e.g., 40).
